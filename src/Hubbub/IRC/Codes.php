...
/**
 * Protocol code trait for user types, channel types, and protocol numeric definitions (such as 001 for RPL_WELCOME).
 *
 * This file is unconventionally organized due to it's large size.  These lists were generated using Simon Butcher's protocol list merged with our own
 * self-made list.  It has been manually edited, heavily.
 *
 * IRCd's:
 * ircu is Undernet
 * Bahamut is DALnet
 * Unreal claims a market share of 50%
 * Freenode uses a modified version of charybdis, basically (ircd-seven)
 *
 * @see     http://www.efnet.org/?module=docs&doc=13
 * @package Hubbub\IRC
 */
trait Codes {

    /**
     * Converts a numeric such as '001' (as a string) or 1 (as an integer) into the string representation, such as RPL_WELCOME in the example given.
     *
     * @param string|int $numeric The numeric to parse, as a left-padded string ('001') or a plain integer (1);
     *
     * @return bool|string The command equiv. of the numeric, such as RPL_WELCOME, or false on failure.
     */
    public function numericToCommand($numeric) {
        if(strlen($numeric) < 3) {
            // Make sure if an integer is passed it is padded to a 3-byte string
            $numeric = str_pad($numeric, 3, '0', STR_PAD_LEFT);
        }

        if(isset($this->commandNumerics[$numeric])) {
            return $this->commandNumerics[$numeric];
        } else {
            return false;
        }
    }
    ...
    /**
     * @param  string $command A command such as RPL_WELCOME to be converted into it's numeric representation.
     *
     * @return string          A string containing the left-padded numeric representation of the command.
     */
    public function commandToNumeric($command) {
        return array_search($command, $this->commandNumerics, true);
    }

    /**
     * Returns a user type (in the context of a channel) given the nickname or prefix alone.  eg. '+Billy' or '+' will return 'VOICED'.
     *
     * @param string $name The full nickname, with prefix, or the prefix itself, such as '+Billy' or '+'.
     *
     * @return bool|string The user type. Either VOICED, HALF_OPERATOR, OPERATOR, SERVICE, or FOUNDER.  Returns false if not known.
     */
    public function getUserType($name) {
        $prefix = substr($name, 0, 1);
        if(isset($this->userTypePrefixes[$prefix])) {
            return $this->userTypePrefixes[$prefix];
        } else {
            return false;
        }
    }
    ...
    /**
     * Returns a string representing the 'channel type' (visibility) given a channel name or prefix alone.  eg. '#hubbub' or '#' will return 'NETWORK'.
     *
     * @param string $chan The full channel name or the type prefix, such as '#' or '#hubbub'
     *
     * @return bool|string The channel type. Either LOCAL, NETWORK, NETWORK_SAFE, NETWORK_UNMODERATED, SOFT_CHAN, or GLOBAL.  Returns false if not known.
     */
    public function getChannelType($chan) {
        $prefix = substr($chan, 0, 1);
        if(isset($this->chanTypePrefixes[$prefix])) {
            return $this->chanTypePrefixes[$prefix];
        } else {
            return false;
        }
    }
    ...
    /**
     * An internal list of user mode prefixes corresponding to their string representations, for example '+' is 'VOICED'
     *
     * @var array
     */
    protected $userTypePrefixes = [
        '+' => 'VOICED',
        '%' => 'HALF_OPERATOR',
        '@' => 'OPERATOR',
        // @ can also map to OWNER with NETWORK_SAFE channels
        '!' => 'SERVICE',
        '.' => 'FOUNDER',
    ];

    /**
     * An internal list of channel type prefixes corresponding to their string representations, for example '#' is 'NETWORK'
     *
     * @var array
     */
    protected $chanTypePrefixes = [
        '&' => 'LOCAL',
        '#' => 'NETWORK',
        '!' => 'NETWORK_SAFE',
        '+' => 'NETWORK_UNMODERATED',
    ...
    /**
     * A very large, long table of 'NUMERIC' => 'PROTOCOL_CONSTANT' table, a result of Simon Butcher's List merged with A.B.'s own personal list.
     * Commented-out lines have not been vetted.  They could be valid, deprecated, very rare/special purpose, or no longer used at all.
     *
     * @var array
     */
    protected $commandNumerics = [
        /*
        ...
        '004' => 'RPL_MYINFO', // RFC2812 + Extended format in KineIRCd
        '005' => 'RPL_ISUPPORT', // RFC2812 is RPL_BOUNCE (Deprecated), parser can correct itself deprecated form is used
        // '006' => 'RPL_MAP', // Unreal.
        // '007' => 'RPL_MAPEND', // Unreal.
        // '008' => 'RPL_SNOMASK', // ircu.
        // '009' => 'RPL_STATMEMTOT', // ircu.
        '010' => 'RPL_BOUNCE',
        // '010' => 'RPL_STATMEM', // Deprecated. ircu.
        ...
        '210' => 'RPL_TRACERECONNECT', // Deprecated. RFC2812.
        // '210' => 'RPL_STATS', // aircd.
        ...
        '231' => 'RPL_SERVICEINFO', // Deprecated. RFC1459.
        '232' => 'RPL_ENDOFSERVICES', // Deprecated. RFC1459.
        // 232 would likely be better off as RPL_RULES?
        // '232' => 'RPL_RULES', // Unreal.
        '233' => 'RPL_SERVICE', // Deprecated. RFC1459.
        ...
        // [244] Original: RPL_STATSSLINE in Hubbub's implementation
        '244' => 'RPL_STATSHLINE', // RFC1459.
        // '245' => 'RPL_STATSSLINE', // Bahamut, IRCnet, Hybrid.
        '246' => 'RPL_STATSPING', // Deprecated. RFC2812.
        ...
        '301' => 'RPL_AWAY', // RFC1459.  KineIRCd has an additional param.
        ...
        '316' => 'RPL_WHOISCHANOP', // Deprecated. RFC1459.
        ...
        '321' => 'RPL_LISTSTART', // Deprecated. RFC1459.
        ...
        '342' => 'RPL_SUMMONING', // Deprecated. RFC1459.
        ...
        '361' => 'RPL_KILLDONE', // Deprecated. RFC1459.
        '362' => 'RPL_CLOSING', // Deprecated. RFC1459.
        '363' => 'RPL_CLOSEEND', // Deprecated. RFC1459.
        ...
        '384' => 'RPL_MYPORTIS', // Deprecated. RFC1459.
        ...
        '391' => 'RPL_TIME', // RFC1459.  ircu, bdq-ricd, all have unique formats :(
        ...
        '431' => 'ERR_NONICKNAMEGIVEN', // Registration. RFC1459.
        '432' => 'ERR_ERRONEUSNICKNAME', // Registration. RFC1459.
        '433' => 'ERR_NICKNAMEINUSE', // Registration. RFC1459.
        ...
        '438' => 'ERR_NICKTOOFAST', // ircu.  DALNet claims they did it first =)
        // '438' => 'ERR_DEAD', // IRCnet.
        ...
        // '449' => 'ERR_NOTIMPLEMENTED', // Registration. Undernet.
        '451' => 'ERR_NOTREGISTERED', // Registration. RFC1459.
        // '452' => 'ERR_IDCOLLISION',
        // '453' => 'ERR_NICKLOST',
        ...
        '461' => 'ERR_NEEDMOREPARAMS', // Registration. RFC1459.
        // 462 was typo'd in Hubbub's original list
        '462' => 'ERR_ALREADYREGISTERED', // Registration. RFC1459.
        '463' => 'ERR_NOPERMFORHOST', // RFC1459.
        '464' => 'ERR_PASSWDMISMATCH', // Registration. RFC1459.
        '465' => 'ERR_YOUREBANNEDCREEP', // RFC1459.
        '466' => 'ERR_YOUWILLBEBANNED', // Deprecated. RFC1459.
        ...
        // [477] I've only seen ERR_NEEDREGGEDNICK in production.
        ...
        '492' => 'ERR_NOSERVICEHOST', // Deprecated. RFC1459.
        ...
        // '503' => 'ERR_VWORLDWARN', // Deprecated. AustHex.
        // '504' => 'ERR_USERNOTONSERV',
        // '511' => 'ERR_SILELISTFULL', // ircu.
        // '512' => 'ERR_TOOMANYWATCH', // Bahamut.
        // '513' => 'ERR_BADPING', // Registration. ircu.
        // '514' => 'ERR_INVALID_ERROR', // ircu.
        // '514' => 'ERR_TOOMANYDCC', // Bahamut (+ Unreal?).
        ...
        // '520' => 'ERR_WHOTRUNC', // Deprecated. AustHex.
        ...
        // '617' => 'RPL_DCCSTATUS', // Bahamut ( + Unreal?).
        // '617' => 'RPL_WHOISBOT', // Ultimate.
        // '618' => 'RPL_DCCLIST', // Bahamut (+ Unreal?).
        // '619' => 'RPL_ENDOFDCCLIST', // Bahamut (+ Unreal?).
        // '619' => 'RPL_WHOWASHOST', // Ultimate.
        // '620' => 'RPL_DCCINFO', // Bahamut (+ Unreal?).
        ...
        // '640' => 'RPL_DUMPING', // Deprecated. Unreal.
        // '641' => 'RPL_DUMPRPL', // Deprecated. Unreal.
        // '642' => 'RPL_EODUMP', // Deprecated. Unreal.
        ...
        // '740' => 'RPL_CHALLENGE_TEXT', // RatBox.
        // '741' => 'RPL_CHALLENGE_END', // RatBox.
        ...
        // '903' => 'RPL_SASL', // charybdis.
        // '904' => 'ERR_SASL', // charybdis.
        // '972' => 'ERR_CANNOTDOCOMMAND', // Unreal.
        // '973' => 'ERR_CANNOTCHANGEUMODE', // KineIRCd.
        // '974' => 'ERR_CANNOTCHANGECHANMODE', // KineIRCd (+ Unreal?).
